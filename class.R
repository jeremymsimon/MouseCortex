# S4 class file for Loo et al., "Single-cell transcriptomic catalog of mouse cortical development", bioRxiv, 2017
# Nearly all of this is derived from Shekhar et al., "Comprehensive classification of retinal bipolar cells using single-cell transcriptomics", Cell, 2016, using the version pushed to Github on October 19th, 2016
...
scDrop <- setClass("scDrop", slots = 
                     c(count.data="data.frame", data="data.frame",batch.data="data.frame", scale.data="matrix", 
                       group="vector", pca.load="data.frame",pca.scores="data.frame",pca.eigenvalues="data.frame",
                       meta="data.frame",tsne.y="data.frame", cell.names="vector",sils="vector",numclust="vector"))
...
setGeneric("perform_refined_clustering", function(object,pcs.use=1:10, corMethod="spearman", filePrefix="DropSeqAnalysis", min.newcluster.size=10, min.proportion.outliers=0.10, optimize.NN=TRUE, opt.NN=30) standardGeneric("perform_refined_clustering"))
setMethod("perform_refined_clustering", "scDrop", function(object,pcs.use=1:10, corMethod="spearman", filePrefix="DropSeqAnalysis", min.newcluster.size=10, min.proportion.outliers=0.10, optimize.NN=TRUE, opt.NN=30) {
	...
	if(optimize.NN==TRUE) {
		...
		pdf(file=pdfName)
		print(ggplot(as.data.frame(object@sils), aes(x=seq(1:100),y=object@sils),useDingbats=FALSE) + geom_point() + geom_smooth(span=0.5) + scale_x_continuous(breaks=c(seq(0,100,10)),limits=c(0,100),minor_breaks=NULL) + scale_y_continuous(breaks=c(seq(min.sils,max.sils,0.01)),limits=c(min.sils,max.sils),minor_breaks=NULL))
		print(ggplot(as.data.frame(object@numclust), aes(x=seq(1:100),y=object@numclust),useDingbats=FALSE) + geom_point() + geom_smooth(span=0.2) + scale_x_continuous(breaks=c(seq(0,100,10)),limits=c(0,100),minor_breaks=NULL) + scale_y_continuous(breaks=c(seq(min.clust,max.clust,5)),limits=c(min.clust,max.clust),minor_breaks=NULL))
		dev.off()
	}
